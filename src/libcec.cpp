...
Cec::Cec(CecCallback * callback)
{
    ...
    gethostname(config.strDeviceName, sizeof(config.strDeviceName));
...
void Cec::init() {
    ...
    else
    {
        ...
        cec->SetConfiguration(&config);
    }
}

cec_logical_address Cec::open(const std::string &name, bool makeActive) {
    LOG4CPLUS_TRACE_STR(logger, "Cec::open()");
    int id = 0;

    config.bActivateSource = makeActive;
...
void Cec::setDeviceAddress(const HDMI::address & address) {
...
void Cec::setDeviceName(const string & name) {
    strncpy(config.strDeviceName, name.c_str(), sizeof(config.strDeviceName));
}
